...
from appointment.api.serializers import ServiceSerializer, WorkingHourSerializer, AppointmentSerializer,DoctorDateSerializer,DoctorSerializer, GetAvailableSlotsSerializer, AvailableSlotResponseSerializer, CreateAppointmentSerializer, AppointmentResponseSerializer, PayDownPaymentSerializer, PayDownPaymentResponseSerializer, CompleteAppointmentSerializer, CanceleAppontmentSerializer, CanceleAppontmnetResponseSerializer, workinghourDetailSerializer
from permissions.permissions import ReadOnlyOrOwnerOrAdmin, OwnerOrAdmin, IsPatient, IsAdmin, ReadOnlyOrPatient, ReadOnlyOrDoctorOrAdmin, OwnerDoctorOrAdmin, DoctorOrAdmin, OwnerUserOrDoctorOrAdmin
...
# the view below creates service object and returns list of services
class ServiceView(generics.ListCreateAPIView):
    queryset = Service.objects.all().order_by("id")
    serializer_class = ServiceSerializer
    permission_classes = [ReadOnlyOrDoctorOrAdmin]
    pagination_class = SmallResultSetPagination
    filter_backends = [SearchFilter]
    search_fields =["service__doctor__user__fullname"]
...
class ServiceDetailView(generics.RetrieveUpdateDestroyAPIView):
    serializer_class = ServiceSerializer
    permission_classes = [ReadOnlyOrOwnerOrAdmin]
    ...
    def get_object(self):
        ...
        self.check_object_permissions(self.request, service_obj)
...
class WorkingHourView(generics.ListCreateAPIView):
    ...
    pagination_class = SmallResultSetPagination
    filter_backends = [SearchFilter]
    search_fields =["workinghour__doctor__user__fullname"]

    def get_queryset(self):
        return WorkingHour.objects.all().order_by("id") 
...
class WorkingHourDetailView(generics.RetrieveUpdateDestroyAPIView):
    serializer_class = workinghourDetailSerializer
    permission_classes = [ReadOnlyOrDoctorOrAdmin]
    ...
    def get_object(self):
        ...
        self.check_object_permissions(self.request, WorkingHour_obj)
...
#create a view to get access to available time slots        
...
#this view canceles appointment by its patient(owner)
...
class AppointmentListOfSpecificPatient(generics.ListAPIView):
    serializer_class = AppointmentSerializer
    permission_classes = [IsAuthenticated, OwnerUserOrDoctorOrAdmin]
...
class AppointmentListOfSpecificDoctor(generics.ListAPIView):
    serializer_class = AppointmentSerializer
    permission_classes = [IsAuthenticated, OwnerDoctorOrAdmin]
    pagination_class = SmallResultSetPagination
    filter_backends = [SearchFilter]
